...
``tweets`` array). ``DATA_DIR`` defaults to ``data/.apps/thread-writer/`` and
...
DATA_DIR = Path(os.environ.get("THREAD_WRITER_DATA_DIR", "data/.apps/thread-writer"))
PORT = int(os.environ.get("THREAD_WRITER_PORT", "8082"))
...
<html lang="en">
<head>
<script>if (window.parent !== window) window.parent.postMessage({{type: "minds-location", path: location.pathname + location.search}}, "*");</script>
<meta charset="utf-8">
...
def _redirect_after_generate(slug: str, error_message: str | None) -> WerkzeugResponse:
    ...
    The target is *relative* to ``/generate`` so it resolves correctly at the
    app's own origin. This mirrors the existing schedule/mark redirect.
    """
...
def generate() -> WerkzeugResponse:
    ...
    # Relative redirect so it resolves at the app's own origin (see _redirect_after_generate).
...
def _redirect_to_calendar(error_message: str | None = None) -> WerkzeugResponse:
    ...
    Relative ("calendar") so it resolves at the app's own origin.
    """
    target = "calendar"
    if error_message:
        target = f"{target}?error={quote(error_message)}"
